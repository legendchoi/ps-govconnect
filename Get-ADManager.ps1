...
# Exchange Properties
$PrimarySMTPAddress = (Get-Mailbox $Uid).PrimarySMTPAddress

# AD Properties
$MailFromAD =        (Get-ADUser $Uid -Properties mail).mail
$UserPrincipalName = (Get-ADUser $Uid).UserPrincipalName

# Display Section
# SAP Related
Write-Host ""
Write-Output "Your Displyname is`t`t: $yourDisplayName"
Write-Output "Your Fullname is`t`t: $yourFullName"
Write-Host "Your CN is`t`t`t: $yourCN"
Write-Host ""
Write-Host "[SAP Related]" -ForegroundColor Yellow
Write-Host "Your SAP ID is`t`t`t: $yourSAPID"
Write-Host "Your WorkForceID is`t`t: $yourWorkForceID"
Write-Host "Your System WorkForceID is`t: $yourSysWorkForceID"
Write-Host "Your SAP WorkForce ID is`t: $yourSAPWorkForceID"
Write-Host "Your OrgUnit is`t`t`t: $yourOrgUnit" -BackgroundColor Red
# Email
Write-Host "[Email]" -ForegroundColor Yellow
Write-Host "UserPrincipalName from AD`t: $UserPrincipalName"
Write-Host "PrimarySMTPAddress from Exchange: $PrimarySMTPAddress"
Write-Host "Your Mail in AD`t`t`t: $MailFromAD"
Write-Host "Your Mail in meta (IDM)`t`t: $yourEmail $yourEmailExist"
Write-Host "Your SAP Email is `t`t: $yourSapEmail $yourSapEmailExist"
Write-Host "Your System Email All is`t: $yourSystemEmailAll $yourSystemEmailAllExist"
Write-Host "Your PM Mail is`t`t`t: $yourPMMail $yourPMMailExist"
Write-Host "Your AD Alias is`t`t: $yourADAlias $yourADAliasExist"
# Forgot Password
...
if ($yourADManager -ne $null) {
    ...
    Write-Host "Your AD Manager is`t`t: $yourADManagerFullName($yourADManager) - $yourADManagerEmail"
} else {
...
if ($yourSAPManager -ne $null) {
    ...
    Write-Host "Your SAP Manager is`t`t: $yourSAPManagerFullName($yourSAPManager) - $yourSAPManagerEmail"
} else {
...
if ( ($yourEmail -ne $null -or $yourSapEmail -ne $null) -and ($yourForgotPWEnableExist -eq $false -or $yourForgotEmailExist -eq $false)) {
    ...
    Write-Host "Your Email address`t`t: $yourEmail"
