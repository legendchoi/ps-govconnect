    ...
    do {
        ...
        if (!$EmailBoxName) {
            ...
            if ($EmailBoxName) {
                ...
                try {
                    if ($OperationChoice -eq 1) {
                        # $MailBoxList = Get-Mailbox -Filter "(Name -like '*$EmailBoxName*') -or (Alias -like '*$EmailBoxName*') -or (EmailAddresses -like '*$EmailBoxName*')" -RecipientTypeDetails UserMailbox -ErrorAction Stop
                        ...
                        # Write-Host "Hey before IF"
                        if ($EmailBoxName -match "@") {
                            # Write-Host "Yes @ matched!"
                            # Write-Host "Email Box Name: $EmailBoxName"
                            $MailBoxList = Get-Mailbox $EmailBoxName -RecipientTypeDetails SharedMailbox -ErrorAction Stop
                            # Write-Host "Mail Box List: $MailBoxList"
                        } else {
                            # $MailBoxList = Get-Mailbox "*$EmailBoxName*" -RecipientTypeDetails SharedMailbox -ErrorAction Stop
                            $MailBoxList = Get-Mailbox -Filter "(Name -like '*$EmailBoxName*') -or (Alias -like '*$EmailBoxName*') -or (EmailAddresses -like '*$EmailBoxName*')" -RecipientTypeDetails SharedMailbox -ErrorAction Stop
                        }
