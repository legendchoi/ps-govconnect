...
function Remove-FullAccess {
    ...
    try { 
        Remove-MailboxPermission $EmailBoxName -User $UserName -AccessRights FullAccess -InheritanceType All -ErrorAction Stop -WarningAction Stop | Out-Null 
...
function Remove-SendAsAccess {
    ...
    try { 
        Get-Mailbox $EmailBoxName | Remove-ADPermission -User $UserName -ExtendedRights "Send As" -ErrorAction Stop -WarningAction Stop 
...
function Show-MenuMailDelegation {
    ...
    Write-Host "$EmailBoxName - $EmailBoxAddress" -ForegroundColor Green
...
do {
    ...
    $EmailBoxName   = Select-Mailbox -EmailBoxName $EmailBoxName -OperationChoice $OperationChoice # will retun mailbox alias
    $UserName       = Select-User -UserName $UserName
    ...
    do {
        ...
        Switch ($Access) {
            ...
            "addfullsendon" # Set-Both (Full & SendOnBehalf)
            {
                $AddedFull = Set-FullAccess -EmailboxName $EmailBoxName -UserName $UserName
            ...
            "delfullsendon" # Remove-Both (Full & SendOnBehalf)
            {
                $Removedfull = Remove-FullAccess -EmailboxName $EmailBoxName -UserName $UserName
            ...
            default 
            { 
                # This statement will never be ran but I leave this just in case
